...
const footerCopy: Record<Language, { line: string; visit: string; explore: string; reserve: string; rights: string; faq: string; manage: string; navigation: string; home: string }> = {
  en: { line: 'Venetian cooking, warm hospitality and a hidden garden near Rialto.', visit: 'Visit', explore: 'Explore', reserve: 'Reserve your table', rights: 'All rights reserved', faq: 'FAQ', manage: 'My bookings', navigation: 'Footer navigation', home: 'Al Gobbo di Rialto, back to the home page' },
  ...
  fr: { line: 'Cuisine vénitienne, accueil chaleureux et jardin caché près du Rialto.', visit: 'Nous trouver', explore: 'Explorer', reserve: 'Réserver une table', rights: 'Tous droits réservés', faq: 'Questions fréquentes', manage: 'Mes réservations', navigation: 'Navigation de pied de page', home: 'Al Gobbo di Rialto, retour à l’accueil' },
  de: { line: 'Venezianische Küche, herzliche Gastfreundschaft und ein versteckter Garten am Rialto.', visit: 'Besuchen', explore: 'Entdecken', reserve: 'Tisch reservieren', rights: 'Alle Rechte vorbehalten', faq: 'Häufige Fragen', manage: 'Meine Reservierungen', navigation: 'Fußnavigation', home: 'Al Gobbo di Rialto, zurück zur Startseite' },
  es: { line: 'Cocina veneciana, cálida hospitalidad y un jardín escondido cerca de Rialto.', visit: 'Visítanos', explore: 'Explorar', reserve: 'Reservar mesa', rights: 'Todos los derechos reservados', faq: 'Preguntas frecuentes', manage: 'Mis reservas', navigation: 'Navegación del pie de página', home: 'Al Gobbo di Rialto, volver al inicio' },
};
  ...
  return (
    <footer className="border-t border-white/10 bg-[#11100e] text-white">
      <div className="mx-auto max-w-[1480px] px-4 py-14 sm:px-7 sm:py-20 lg:px-10">
        ...
        <div className="flex flex-col gap-3 pt-6 text-xs font-semibold uppercase tracking-[0.12em] text-white/60 sm:flex-row sm:items-center sm:justify-between"><p>© {new Date().getFullYear()} Al Gobbo di Rialto · {copy.rights}</p><div className="flex flex-wrap gap-5"><Link to="/privacy" className="min-h-11 py-3 hover:text-white">Privacy</Link><Link to="/my-reservations" className="min-h-11 py-3 hover:text-white">{copy.manage}</Link></div></div>
      </div>
    </footer>
  );
}

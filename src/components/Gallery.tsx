...
function GalleryModal({ group, onClose }: { group: GalleryGroup; onClose: () => void }) {
  ...
  return (
    <motion.div ref={dialogRef} tabIndex={-1} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] grid place-items-center bg-venetian-brown/95 p-4 backdrop-blur-md" onClick={onClose} role="dialog" aria-modal="true" aria-label="Photo gallery">
      <button ref={closeButtonRef} type="button" onClick={onClose} className="absolute right-5 top-5 grid h-12 w-12 place-items-center border border-white/25 text-white hover:border-venetian-gold hover:text-venetian-gold" aria-label="Close gallery"><X className="h-5 w-5" /></button>
...
export default function Gallery() {
  ...
  const { t } = useLanguage();
  const closeGallery = useCallback(() => setSelectedGroup(null), []);
  ...
  return (
    <>
      ...
      <AnimatePresence>{selectedGroup ? <GalleryModal group={selectedGroup} onClose={closeGallery} /> : null}</AnimatePresence>
    </>
  );
}
